  ...
  return (
    <div className="flex w-full flex-col gap-8">
      <PageHeader
        eyebrow="Outfits"
        title="手动搭配工作台"
        description={`${session.displayName} 可以从当前身份已有的衣物中手动组合搭配。列表会直接展开整套预览，让你同时看到主体、鞋子和配饰。`}
        actions={
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="surface-panel rounded-[1.8rem] p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-muted">
            每个预设身份都拥有独立的搭配列表、会话状态和整套搭配展示结果。
          </p>
        </div>
        ...
        <div className="surface-panel rounded-[1.8rem] p-5">
          ...
          <p className="mt-3 text-3xl font-semibold text-foreground-strong">{outfits.length}</p>
          <p className="mt-2 text-sm leading-6 text-muted">
